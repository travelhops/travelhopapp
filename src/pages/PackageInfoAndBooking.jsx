...
const PackageInfoAndBooking = ()=>{
    ...
    return (
        <Layout>
            {Object.keys(singlePackage).length > 0?(
                <MainComponent pkg={singlePackage}/>
            ):<DataLoading/> }
        </Layout>
    );
}
...
const MainComponent = (props)=>{
    ...
    const incrPax = (event)=>{
        ...
        if(pax == props.pkg.paxSize.maxPax)
            return;
        setPax(pax+1);
    }
    ...
    const decrPax = (event)=>{
        ...
        if(pax == props.pkg.paxSize.minPax)
            return;
        setPax(pax-1);
    }
    ...
    const backgroundImageCss = {
        backgroundImage: `linear-gradient(rgba(0,0,0,0.3), rgba(0,0,0,0.3)), url('${props.pkg.bannerUrl}')`,
    ...
    return (
        ...
        <main className="w-full flex justify-center items-end" style={backgroundImageCss}>
            <div className="w-[90%] sm:w-[80%] flex justify-between items-end py-8">
                <div>
                    <h1 className="text-white text-4xl min-[430px]:text-5xl sm:text-6xl min-[900px]:text-7xl font-bold">{props.pkg.destination}</h1>
                    <h4 className="text-white text-2xl min-[430px]:text-3xl sm:text-4xl min-[900px]:text-5xl">{props.pkg.country}</h4>
                </div>

                <div>
                    <h2 className="w-fit text-white text-lg min-[430px]:text-xl sm:text-2xl min-[900px]:text-3xl font-bold rounded-full p-4 bg-[#2F6080] ml-auto">₹{props.pkg.price}*/-</h2>
                    <h4 className="text-lg sm:text-xl min-[900px]:text-2xl text-white font-bold">{props.pkg.duration.day} Days / {props.pkg.duration.night} Nights</h4>
                </div>
            </div>
        </main>
        ...
        <main className="w-full flex justify-center py-12">
            <div className="w-[90%] lg:w-[80%] flex justify-center flex-col md:flex-row">
                <div className="flex-1 md:flex-[0_0_60%]">
                    ...
                    {viewComponent.description?<Description description={props.pkg.description} includeExclude={[props.pkg.inclusion, props.pkg.exclusion]}/>:viewComponent.tourPlan?<TourPlan tourPlan={props.pkg.tourPlan}/>:viewComponent.images?<Images/>:""}
                ...
                <div className="w-full relative h-fit flex-1 overflow-hidden md:flex-[0_0_40%] font-arial p-8 py-12 border-[0.25px] border-gray-300 shadow-2xl rounded-[2rem] before:content-[''] before:absolute before:w-[94%] before:h-8 before:bg-[#2F6080] before:top-0 before:left-1/2 before:-translate-x-1/2 before:-translate-y-1/2 before:rounded-full">
                    <form className="bookTour" method="POST">
                        ...
                        <div className="flex flex-col">
                            {/*No. of person*/}
                            <span>No. of {props.pkg.paxType}</span>
        ...
        <div className="w-full md:w-[90%] text-[#508CB7] py-12 [&>div]:pb-12">
            ...
            {props.tourPlan.map((days, index)=>{
                return (<div key={index}>
                    ...
                    <hr className="my-3 border border-solid border-gray-300"/>
                   <ul className="list-disc text-lg min-[900px]:text-xl">
                       {days.map((plan, indx)=>{
